...
function doUpdateTest(){
  ...
  if(isset($_POST['btnUpdateExam'])){
      ...
      $passing_rate = $_POST['passing_rate'];
      $total_questions = $_POST['total_questions'];
      $time_limit = $_POST['time_limit'];

      $stmt = "UPDATE examproper SET 
      ...
      if($conn->exec($stmt)==true){
        ...
        $sql ="SELECT * FROM `questions` WHERE `user_id`='$user_id' AND `Course`='$description' ORDER BY RAND()";
         ...
         while($row = $stmt->fetch()){
          ...
          $sql = "SELECT * FROM `tblexamquestion` WHERE `q_id`='$q_id' AND `access_code`='$access_code'";
          $stmt = $conn->prepare($sql);
          $stmt->execute();  
          $maxrow = $stmt->rowCount();
